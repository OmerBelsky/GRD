...
from pddl_problems.grd_direct import GRDDirectSearch
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Guard-rail planning with a custom direct search (no PDDLStream)"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set, print depth progress every N expansions at that depth.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.load_checkpoint:
        checkpoint_payload = GRDDirectSearch.load_checkpoint_file(args.load_checkpoint)

    planner = GRDDirectSearch(
    ...
    print_plan(result.plan, planner)
    print(f"Expanded nodes: {result.expanded} | timed_out={result.timed_out} | best_cost={result.best_cost}")
    if args.tree_log:
